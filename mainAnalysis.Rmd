...
-read in this season's data up to current date
-write a loop to read in new games
    -something like: assume we're complete to the last date, then check every day going forward
...
sum(shotDB$baseline <= 0)
sum(shotDB$baseline >= 50)

ggplot(data = shotDB,
       aes(x = baseline,
           y = depth,
           col = shotQuadrant)) +
  geom_point(alpha = .5, size = 3.5, shape = 15)
...
for (gameDate in paste0("202211",sprintf("%02d",c(7:15,17)))) {
  date()
  print(gameDate)
